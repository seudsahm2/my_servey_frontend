'use client'; // ensure component runs only in browser (required for DOM/SVG dependent libs like Recharts)

// React + Next imports
import { useEffect, useState } from 'react';               // React hooks for lifecycle and state
import Link from 'next/link';                              // Next Link component for navigation
...
// so we don't use next/dynamic (which expects a component) and avoid the TS mismatch.
type RechartsNamespace = any;                              // loose type for the module namespace

// color palette used for pies/cells
...
export default function Dashboard() {
    ...
    const isRTL = language === 'ar';                         // determine text direction
    const router = useRouter();                              // router instance for navigation
    const { isAuthenticated, loading: authLoading, logout } = useAuth(); // auth state and actions
    ...
    // Helper: check for non-empty arrays
    const hasArray = (arr: any) => Array.isArray(arr) && arr.length > 0;

    // redirect to login if not authenticated once authLoading resolves
    useEffect(() => {
        if (!authLoading && !isAuthenticated) {
            router.push('/admin/login'); // send to login page
        }
    }, [isAuthenticated, authLoading, router]);

    // lazy-load the Recharts module at runtime (client-side only)
    useEffect(() => {
        let mounted = true; // flag to avoid state updates after unmount

        // import wrapper (adjust path if your project doesn't use @ alias)
        import('@/components/RechartsWrapper')
            .then((mod) => {
                const rechartsModule = (mod && (mod.default ?? mod)) as RechartsNamespace; // support default or named export
                if (mounted) setRecharts(rechartsModule); // set module into state
            })
            .catch((err) => {
                console.error('Failed to load RechartsWrapper:', err); // helpful for chunk load or path issues
            });
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                // run requests in parallel, but collect results even if one fails
                const [studentsRes, teachersRes, summaryRes] = await Promise.allSettled([
                    getStudentAnalytics(),
                    getTeacherAnalytics(),
                    getAnalyticsSummary(),
                ]);
                ...
                if (studentsRes.status === 'fulfilled') {
                    setStudentData(studentsRes.value);
                } else {
                    console.error('getStudentAnalytics failed:', studentsRes.reason);
                    if (studentsRes.reason?.response?.status === 401) {
                        logout(); router.push('/admin/login'); return;
                    }
                }

                // handle teachers result
                if (teachersRes.status === 'fulfilled') {
                    setTeacherData(teachersRes.value);
                } else {
                    console.error('getTeacherAnalytics failed:', teachersRes.reason);
                    if (teachersRes.reason?.response?.status === 401) {
                        logout(); router.push('/admin/login'); return;
                    }
                }
                ...
                if (summaryRes.status === 'fulfilled') {
                    ...
                    console.error('getAnalyticsSummary failed:', summaryRes.reason);
                    if (summaryRes.reason?.response?.status === 401) {
                        logout(); router.push('/admin/login'); return;
                    }
                }
            } catch (err) {
    ...
    // determine whether to show data or no-data state
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    {hasData ? (
                        <>
                            ...
                            {activeTab === 'overview' && (
                                <div className="space-y-6 animate-fade-in">
                                    ...
                                    {/* quick insights grid */}
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        {/* student willingness pie (guarded) */}
                                        {studentData && (studentData.total_responses || 0) > 0 && (
                                                        ...
                                                        <Recharts.ResponsiveContainer width="100%" height={240}>
                                                            <Recharts.PieChart>
                                                                <Recharts.Pie
                                                                    ...
                                                                    cy="50%"
                                                                    labelLine={false}
                                                                    label={(entry: any) => `${entry.name}: ${Math.round((entry.value / (studentData.willingness.willing + studentData.willingness.not_willing || 1)) * 100)}%`}
                                                                    outerRadius={80}
                                                                    dataKey="value"
                                                                >
                                                                ...
                                                                <Recharts.Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '12px' }} />
                                                            </Recharts.PieChart>
                                                        </Recharts.ResponsiveContainer>
                                                    ) : (
                                        ...
                                        {/* teacher willingness pie (guarded) */}
                                        {teacherData && (teacherData.total_responses || 0) > 0 && (
                                            <div className="p-6 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 hover:border-white/20 transition-all">
                                                <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
                                                    <span className="text-2xl">🎉</span>
                                                        ...
                                                        <Recharts.ResponsiveContainer width="100%" height={240}>
                                                            <Recharts.PieChart>
                                                                <Recharts.Pie
                                                                    ...
                                                                    cy="50%"
                                                                    labelLine={false}
                                                                    label={(entry: any) => `${entry.name}: ${Math.round((entry.value / (teacherData.platform_interest.would_join + teacherData.platform_interest.would_not_join || 1)) * 100)}%`}
                                                                    outerRadius={80}
                                                                    dataKey="value"
                                                                >
                            ...
                            {/* students tab (charts guarded similarly) */}
                            {activeTab === 'students' && studentData && (studentData.total_responses || 0) > 0 && (
                                <div className="space-y-6 animate-fade-in">
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        ...
                                        <div className="p-6 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 hover:border-white/20 transition-all">
                                            ...
                                            <div style={{ minHeight: 300 }}>
                                                {Recharts && hasArray(studentData.experience_distribution) ? (
                                                    <Recharts.ResponsiveContainer width="100%" height={300}>
                                                        <Recharts.BarChart data={studentData.experience_distribution}>
                                                            <defs>
                                                            ...
                                                            <Recharts.CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                                                            <Recharts.XAxis dataKey="quran_experience" stroke="#94a3b8" />
                                                            <Recharts.YAxis stroke="#94a3b8" />
                                                            <Recharts.Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '12px' }} />
                                                            <Recharts.Bar dataKey="count" fill="url(#colorExp)" radius={[8, 8, 0, 0]} />
                                                        </Recharts.BarChart>
                                                    </Recharts.ResponsiveContainer>
                                                ) : (
                                        ...
                                        <div className="p-6 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 hover:border-white/20 transition-all">
                                            ...
                                            <div style={{ minHeight: 300 }}>
                                                {Recharts && hasArray(studentData.age_distribution) ? (
                                                    <Recharts.ResponsiveContainer width="100%" height={300}>
                                                        <Recharts.BarChart data={studentData.age_distribution}>
                                                            <defs>
                                                            ...
                                                            <Recharts.YAxis stroke="#94a3b8" />
                                                            <Recharts.Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '12px' }} />
                                                            <Recharts.Bar dataKey="count" fill="url(#colorAge)" radius={[8, 8, 0, 0]} />
                                                        </Recharts.BarChart>
                                                    </Recharts.ResponsiveContainer>
                                                ) : (
                                                    <div className="text-gray-400 text-center py-8">No data</div>
                                        ...
                                        <div className="p-6 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 hover:border-white/20 transition-all">
                                            ...
                                            <div style={{ minHeight: 300 }}>
                                                {Recharts && hasArray(studentData.session_length_distribution) ? (
                                                    <Recharts.ResponsiveContainer width="100%" height={300}>
                                                        <Recharts.BarChart data={studentData.session_length_distribution}>
                                                            <defs>
                                                            ...
                                                            <Recharts.CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                                                            <Recharts.XAxis dataKey="preferred_session_length" stroke="#94a3b8" />
                                                            <Recharts.YAxis stroke="#94a3b8" />
                                                            <Recharts.Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '12px' }} />
                                                            <Recharts.Bar dataKey="count" fill="url(#colorSession)" radius={[8, 8, 0, 0]} />
                                                        </Recharts.BarChart>
                                                    </Recharts.ResponsiveContainer>
                                                ) : (
                                                    <div className="text-gray-400 text-center py-8">No data</div>
                                        ...
                                        <div className="p-6 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 hover:border-white/20 transition-all">
                                            ...
                                            <div style={{ minHeight: 300 }}>
                                                {Recharts && hasArray(studentData.time_preference_distribution) ? (
                                                    <Recharts.ResponsiveContainer width="100%" height={300}>
                                                        <Recharts.BarChart data={studentData.time_preference_distribution}>
                                                            <defs>
                                                            ...
                                                            <Recharts.CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                                                            <Recharts.XAxis dataKey="time_preference" stroke="#94a3b8" />
                                                            <Recharts.YAxis stroke="#94a3b8" />
                                                            <Recharts.Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '12px' }} />
                                                            <Recharts.Bar dataKey="count" fill="url(#colorTime)" radius={[8, 8, 0, 0]} />
                                                        </Recharts.BarChart>
                                                    </Recharts.ResponsiveContainer>
                                                ) : (
                                                    <div className="text-gray-400 text-center py-8">No data</div>
                                        ...
                                        <div className="p-6 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 hover:border-white/20 transition-all lg:col-span-2">
                                            ...
                                            <div style={{ minHeight: 300 }}>
                                                {Recharts && hasArray(studentData.subjects_interest) ? (
                                                    <Recharts.ResponsiveContainer width="100%" height={300}>
                                                        <Recharts.BarChart data={studentData.subjects_interest}>
                                                            <defs>
                                                            ...
                                                            <Recharts.CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                                                            <Recharts.XAxis dataKey="subject" stroke="#94a3b8" />
                                                            <Recharts.YAxis stroke="#94a3b8" />
                                                            <Recharts.Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '12px' }} />
                                                            <Recharts.Bar dataKey="count" fill="url(#colorSubjects)" radius={[8, 8, 0, 0]} />
                                                        </Recharts.BarChart>
                                                    </Recharts.ResponsiveContainer>
                                                ) : (
                                                    <div className="text-gray-400 text-center py-8">No data</div>
                            ...
                            {/* teachers tab */}
                            {activeTab === 'teachers' && teacherData && (teacherData.total_responses || 0) > 0 && (
                                <div className="space-y-6 animate-fade-in">
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        {/* teaching background pie */}
                                        <div className="p-6 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 hover:border-white/20 transition-all">
                                            <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
                                                <span className="text-2xl">🕌</span>
                                            ...
                                            <div style={{ minHeight: 300 }}>
                                                {Recharts && hasArray(teacherData.teaching_background_distribution) ? (
                                                    <Recharts.ResponsiveContainer width="100%" height={300}>
                                                        <Recharts.PieChart>
                                                            <Recharts.Pie
                                                                ...
                                                                cy="50%"
                                                                labelLine={false}
                                                                label={(entry: any) => `${entry.teaching_background}: ${Math.round((entry.count / (teacherData.total_responses || 1)) * 100)}%`}
                                                                outerRadius={100}
                                                                dataKey="count"
                                                            >
                                        ...
                                        <div className="p-6 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 hover:border-white/20 transition-all">
                                            ...
                                            <div style={{ minHeight: 300 }}>
                                                {Recharts && hasArray(teacherData.age_distribution) ? (
                                                    <Recharts.ResponsiveContainer width="100%" height={300}>
                                                        <Recharts.BarChart data={teacherData.age_distribution}>
                                                            <defs>
                                                            ...
                                                            <Recharts.YAxis stroke="#94a3b8" />
                                                            <Recharts.Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '12px' }} />
                                                            <Recharts.Bar dataKey="count" fill="url(#colorTeacherAge)" radius={[8, 8, 0, 0]} />
                                                        </Recharts.BarChart>
                                                    </Recharts.ResponsiveContainer>
                                                ) : (
                                                    <div className="text-gray-400 text-center py-8">No data</div>
                                        ...
                                        <div className="p-6 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 hover:border-white/20 transition-all lg:col-span-2">
                                            ...
                                            <div style={{ minHeight: 300 }}>
                                                {Recharts && hasArray(teacherData.confident_topics) ? (
                                                    <Recharts.ResponsiveContainer width="100%" height={300}>
                                                        <Recharts.BarChart data={teacherData.confident_topics}>
                                                            <defs>
                                                            ...
                                                            <Recharts.CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                                                            <Recharts.XAxis dataKey="topic" stroke="#94a3b8" />
                                                            <Recharts.YAxis stroke="#94a3b8" />
                                                            <Recharts.Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '12px' }} />
                                                            <Recharts.Bar dataKey="count" fill="url(#colorTopics)" radius={[8, 8, 0, 0]} />
                                                        </Recharts.BarChart>
                                                    </Recharts.ResponsiveContainer>
                                                ) : (
                                                    <div className="text-gray-400 text-center py-8">No data</div>
                        ...
                        <div className="p-12 bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 text-center animate-fade-in">
                            ...
                            <p className="text-gray-400 mb-8 max-w-md mx-auto">{t.startCollecting}</p>
